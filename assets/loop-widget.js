...
let d= document.querySelector('.loop-custom-button')
var sellingPlannn = window.loopProps[productId].sellingPlan;
  let quantityDiv= document.querySelector('.quantity-wrapper input')
  let quanTity = parseInt(quantityDiv.value)||1
  let quantityInputs = document.querySelectorAll('.quantity-wrapper button');
...
console.log(quanTity)
 if (Object.keys(sellingPlannn).length !== 0) {
     d.innerHTML =loopFormatMoney(sellingPlannn.price*quanTity,true)
}
  else {
     d.innerHTML =loopFormatMoney(variant.price*quanTity,true)
}
},200)
    });
});
